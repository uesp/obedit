...
BOOL CChildFrameScript::OnCreateClient (LPCREATESTRUCT lpcs, CCreateContext* pContext) {
  ...
  m_Splitter.CreateView(0, 0, RUNTIME_CLASS(CObScptView),        CSize(0,300), pContext);
  m_Splitter.CreateView(1, 0, RUNTIME_CLASS(CObScriptErrorView), CSize(0,75),  pContext);
  
  //m_Splitter.SetRowInfo(0, 1000, 50);
  //m_Splitter.SetRowInfo(1, 75, 50);

  pWnd = m_Splitter.GetPane(0, 0);
  if (pWnd->IsKindOf(RUNTIME_CLASS(CObScptView))) m_pScriptView = (CObScptView* ) pWnd;

  pWnd = m_Splitter.GetPane(1, 0);
  if (pWnd->IsKindOf(RUNTIME_CLASS(CObScriptErrorView))) m_pErrorView = (CObScriptErrorView* ) pWnd;
